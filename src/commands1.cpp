...
CON_COMMAND_CHAT(tag, "testtag")
{
	...
	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_CHEATS))
	{
		SetClanTag(player, "[OWNER]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_RESERVATION))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[HELPER]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_GENERIC))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[ADMIN]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_UNBAN))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[MODERATOR]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_CHANGEMAP))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[VETERAN]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_CONFIG))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[MANAGER]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_PASSWORD))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[CO-OWNER]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_SLAY))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[VIP SILVER]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_VOTE))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[VIP GOLD]");
	}

	if (pZEPlayer->IsAdminFlagSet(ADMFLAG_CONVARS))
	{
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX"Tag added!.");
		SetClanTag(player, "[VIP EMERALD]");
	}
...
CON_COMMAND_CHAT(setcollisiongroup, "set a player's collision group")
{
	...
	for (int i = 0; i < iNumClients; i++)
	{
		...
		ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIXS "Setting collision group on %s from %d to %d.", pTarget->GetPlayerName(), oldgroup, group);
	}
}
