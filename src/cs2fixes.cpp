...
void CS2Fixes::Hook_PostEvent(CSplitScreenSlot		   nSlot,
							  bool					   bLocalOnly,
							  int					   nClientCount,
...
void CS2Fixes::Hook_CheckTransmit(CCheckTransmitInfo**		   ppInfoList,
								  int						   infoCount,
								  ...
								  bool						   bEnablePVSBits)
{
	...
	for (int i = 0; i < infoCount; i++)
	{
		...
		for (int j = 0; j < gpGlobals->maxClients; j++)
		{
			...
			if (!g_bFlashLightTransmitOthers && pFlashLight && !(pSelfController->GetPawnState() == STATE_OBSERVER_MODE && pSelfController->GetObserverTarget() == pController->GetPawn()))
			{
