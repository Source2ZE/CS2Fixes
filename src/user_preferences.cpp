...
CON_COMMAND_F(cs2f_user_prefs_api, "API for user preferences, currently a REST API.", FCVAR_LINKED_CONCOMMAND | FCVAR_SPONLY)
{
	...
	if (args.ArgC() < 2)
		...
		g_pUserPreferencesSystem->SetPreferencesAPIUrl(args[1]);
	}
}
...
void UserPreferencesHttpCallback(int iSlot, uint64 iSteamID, json data)
{
	if (g_pUserPreferencesSystem->PutPreferences(iSlot, iSteamID, data)) {
...
bool CUserPreferencesSystem::PutPreferences(int iSlot, uint64 iSteamID, json preferenceData)
{
	...
	for (auto it = preferenceData.begin(); it != preferenceData.end(); ++it) {
...
void CUserPreferencesSystem::PullPreferences(int iSlot) 
{
	...
	g_HTTPManager.GET(sUserPreferencesUrl, [iSlot, iSteamId](HTTPRequestHandle request, json data){
		UserPreferencesHttpCallback(iSlot, iSteamId, data);
	});
}
...
void CUserPreferencesSystem::PushPreferences(int iSlot)
{
	...
	FOR_EACH_MAP(m_mPreferencesMaps[iSlot], i) {
		uint32 iKeyHash = m_mPreferencesMaps[iSlot].Key(i);
		int iValueIdx = m_mPreferencesMaps[iSlot].Find(iKeyHash);
		if (iValueIdx == m_mPreferencesMaps[iSlot].InvalidIndex())
			continue;
		CPreferenceValue prefValue = m_mPreferencesMaps[iSlot][iValueIdx];
	...
	g_HTTPManager.POST(sUserPreferencesUrl, sDumpedJson, [iSlot, iSteamId](HTTPRequestHandle request, json data){
		UserPreferencesHttpCallback(iSlot, iSteamId, data);
	});
}