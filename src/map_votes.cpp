...
CON_COMMAND_CHAT(maplist, "- List the maps in the server")
{
	...
	ClientPrint(player, HUD_PRINTCONSOLE, "Lista kaikista kartoista:");
	ClientPrint(player, HUD_PRINTTALK, CHAT_PREFIX "Lista kaikista kartoista:");
...
{
	if (iMapIndex >= m_vecMapList.Count() || iMapIndex < 0)
		return false;
	if (m_vecLastPlayedMapIndexes.HasElement(iMapIndex))
		return false;
...
void CMapVoteSystem::StartVote()
{
	...
	for (int i = 0; i < m_vecMapList.Count(); i++)
	{
		if (!IsMapIndexEnabled(i))
			continue;
		if (vecIncludedMaps.HasElement(i))
			continue;
...
void CMapVoteSystem::RegisterPlayerVote(CPlayerSlot iPlayerSlot, int iVoteOption)
{
	...
	if (!pController || !m_bIsVoteOngoing)
		return;
	if (iVoteOption < 0 || iVoteOption >= 10)
		return;

	// Filter out votes on invalid maps
	int iMapIndexToVote = g_pGameRules->m_nEndMatchMapGroupVoteOptions[iVoteOption];
	if (iMapIndexToVote < 0 || iMapIndexToVote >= m_vecMapList.Count())
		return;
...
int CMapVoteSystem::WinningMapIndex()
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (arrMapVotes[i] == iMaxVotes)
		{
			if (iWinningMapCount == iWinningMapTieBreak)
				return i;
			iWinningMapCount++;
		}
	}
...
{
	if (m_bIsVoteOngoing)
		return NominationReturnCodes::VOTE_STARTED;
	if (m_iForcedNextMapIndex != -1 || m_iMaxNominatedMaps == 0)
		return NominationReturnCodes::NOMINATION_DISABLED;

	CCSPlayerController *pController = CCSPlayerController::FromSlot(iPlayerSlot);
	if (!pController)
		return NominationReturnCodes::INVALID_INPUT;
	int iSlot = pController->GetPlayerSlot();
	...
	if (iFoundIndex == -1)
		return NominationReturnCodes::MAP_NOT_FOUND;
	if (!IsMapIndexEnabled(iFoundIndex))
		return NominationReturnCodes::INVALID_MAP;
